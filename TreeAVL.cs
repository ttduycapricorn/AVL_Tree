...
namespace AVL_Tree
{
    class TreeAVL
    {
        ...
        // Can Bang Cay khi bi lech ve ben trai
        ...
        // Can Bang Cay khi bi lech ve ben phai
        ...
        int Height()
        {
            return Get_Height(ref root);
        }

        public int Get_Height(ref Node root)
        {
            if (root == null ||( root.pLeft == null && root.pRight == null))
                return 0;
            else
            {
                int Height_Left = Get_Height(ref root.pLeft);
                int Height_Right = Get_Height(ref root.pRight);
        ...
        public int insertNode(ref Node root, int key, TreeAVL T, DrawTreeAVL draw, Graphics graph, Bitmap bm, Form1 f)
        {
            int res;
            if (root != null)
            {
                ...
                if (root == T.root)
                {
                    ...
                    draw.MoveNode(key, ref x, ref y, root.x, root.y, T, graph, bm, f);
                    Thread.Sleep(200);
                }
                ...
                if (root.key > key)
                {
                    if (root.pLeft != null)
                    {
                        draw.MoveNode(key, ref x, ref y, root.pLeft.x, root.pLeft.y, T, graph, bm, f);
                    }
                    else
                    {
                        ...
                        yAdd = y + 80;
                        xFatherAdd = root.x;
                        draw.MoveNode(key, ref x, ref y, xAdd, yAdd, T, graph, bm, f);
                    ...
                    res = insertNode(ref root.pLeft, key, T, draw, graph, bm, f);
                    if (res < 2) return res;
                    switch (root.Blance)
                    {
                        case 1:
                ...
                else
                {
                    if (root.pRight != null)
                    {
                        draw.MoveNode(key, ref x, ref y, root.pRight.x, root.pRight.y, T, graph, bm, f);
                    }
                    else
                    {
                        xAdd = root.x + Convert.ToInt32(Math.Abs(root.x - root.Father) / 2);
                    ...
                    res = insertNode(ref root.pRight, key, T, draw, graph, bm, f);
                    if (res < 2) return res;
                    switch (root.Blance)
                    {
                        case -1:
        ...
        // Ham Xoa Node
        public int delNode(ref Node root, int key, TreeAVL T, DrawTreeAVL draw, Graphics graph, Bitmap bm, Form1 f)
        {
            if (root == T.root)
            {
            ...
            if (root.key > key)
            {
                ...
                res = delNode(ref root.pLeft, key, T, draw, graph, bm, f);
                if (res < 2) return res;
                switch (root.Blance)
                {
                    case -1:
            ...
            if (root.key < key)
            {
                ...
                res = delNode(ref root.pRight, key, T, draw, graph, bm, f);
                if (res < 2) return res;
                switch (root.Blance)
                {
                    case 1:
            ...
            else
            {
                ...
                { //T CÓ CẢ 2 CON
                    res = searchStandFor(ref p, ref root.pRight, T, draw, graph, bm, f);
                    if (res < 2) return res;
                    switch (root.Blance)
                    {
                        case 1:
        ...
        private int searchStandFor(ref Node p, ref Node q, TreeAVL T, DrawTreeAVL draw, Graphics graph, Bitmap bm, Form1 f)
        {
            Thread.Sleep(300);
            graph.Clear(Color.Honeydew);
            draw.DrawTree(T, graph, bm, f);
            draw.DrawSelectNode_1(p, graph, bm, f);
            ...
            if (q.pLeft != null)
            {
                res = searchStandFor(ref p, ref q.pLeft, T, draw, graph, bm, f);
                if (res < 2) return res;
                switch (q.Blance)
                {
                    case -1:
        ...
        public void HieuUng(Node q, DrawTreeAVL draw, Graphics graph, Bitmap bm, Form1 f)
        {
            graph.FillRectangle(Brushes.Yellow, q.x - f.r, q.y - f.r, 2 * f.r, 2 * f.r);
            graph.FillRectangle(Brushes.WhiteSmoke, q.x - f.R, q.y - f.R, 2 * f.R, 2 * f.R);
            ...
            graph.FillRectangle(Brushes.Red, q.x - f.r, q.y - f.r, 2 * f.r, 2 * f.r);
            draw.DrawNode(q, graph, bm, f);
            graph.FillRectangle(Brushes.IndianRed, q.x - f.R, q.y - f.R, 2 * f.R, 2 * f.R);
            graph.DrawString(q.key.ToString(), new Font(FontFamily.GenericMonospace, 12, FontStyle.Bold), Brushes.Black, q.x - f.r / 2, q.y - f.r / 2);
